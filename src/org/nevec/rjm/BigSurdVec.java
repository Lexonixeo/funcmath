...
public class BigSurdVec implements Comparable<BigSurdVec>
{
        ...
        */
        protected void normalize()
        {
                ...
                */
                ...
                /* add j-th element to the existing vector and combine where possible
                */
        ...
        * @return 0 or +-1.
        * @since 2012-02-15
        * @since 2020-03-01 patched bug in the case of 3 terms
        */
        public int signum()
        {
                ...
                */
                ...
                for( ; offsig < terms.size() ; offsig++)
                        if ( terms.elementAt(offsig).signum() != sig0)
                                break;
                /* All terms had a common sign: take that one
                */
                ...
                * other side and square both sides (which loses the sign) to remove all but one surds.
                ...
                */
                ...
                {
                        if ( offsig == 2)
                                /* 0 and 1 the same sign, 2 a different sign
                                */
                                ;
                        else if ( sig0 == terms.elementAt(2).signum() )
                        {
                                /* 0 and 2 the same sign. 0 and 1 different sign. offsig==1
                                */
                                ;
                        }
                        else
                        {
                                /* 1 and 2 the same sign. 0 and 1 different sign.
                                */
                                offsig = 0 ;
                        }
                        /* build a set of the two elements with the same sign
                        */
                        BigSurdVec lhs = new BigSurdVec( terms.elementAt( (offsig+1)%3 ), terms.elementAt((offsig+2)%3)) ;

                        /* Repeatedly square and isolate remaining square roots until this is
                        * a comparision of Rational elements. lhs.sqr() is a sum of a Rational and a BigSurd
                        */
                        Rational rhs = terms.elementAt(offsig).sqr() ;
                        lhs = lhs.sqr().subtract(rhs) ;
                        /* Now lhs has only two elements, one Rational and one BigSurd, so this
                        * branches into the earlier signature of signum().
                        */
                        switch( lhs.signum() )
                        {
                        case 0:
                                return 0 ;
                        case 1:
                                return terms.elementAt((offsig+1)%3).signum() ;
                        default:
        ...
        * @return The new value representing this-val.
        */
        public BigSurdVec subtract(final Rational val)
        {
                return subtract(new BigSurd(val,Rational.ONE)) ;
        } /* subtract */
