...
public class BigIntegerMath
{
        ...
        */
        ...
        /** Evaluate floor((p*sqrt(D)+q)/r).
        * @param D The non-negative argument.
        * @param p The factor in front of the square root in the numerator.
        * @param q The summand in the numerator
        * @param r The non-zero denominator
        * @return The integer square root, the square root rounded down.
        * @since 2016-03-19
        */
        static public BigInteger isqrt(final BigInteger D, final BigInteger p, final BigInteger q, final BigInteger r)
        {
                if ( D.compareTo(BigInteger.ZERO) < 0 )
                        throw new ArithmeticException("Negative argument "+ D.toString()) ;
                if ( r.compareTo(BigInteger.ZERO) == 0 )
                        throw new ArithmeticException("Zero denominator "+ r.toString()) ;

                if ( p.compareTo(BigInteger.ZERO) == 0  || D.compareTo(BigInteger.ZERO)== 0)
                        /* degenerate case, return floor(q/r) */
                        return q.divide(r) ;

                /* eliminate common sign if r<0
                */
                if ( r.compareTo(BigInteger.ZERO) < 0 )
                        return isqrt(D,p.negate(),q.negate(),r.negate()) ;

                /* Need to solve (p*sqrt(D)+q)/r = x.
                * So (r*x-q)^2 - p^2*D =0.
                */

                /* a rough approximate value of sqrt(D)
                */
                final int bl = D.bitLength() ;
                final BigInteger sqrtD = D.shiftRight(bl/2-1) ;

                /* starting value of the return value x.
                */
                BigInteger x = p.multiply(sqrtD).add(q).divide(r) ;

                final BigInteger p2D = p.pow(2).multiply(D) ;
                final BigInteger twor = r.multiply(BigInteger.valueOf(2)) ;

                BigInteger[] xstack = new BigInteger[2] ;
                xstack[1] = x ;
                while ( true)
                {
                        BigInteger rxq = r.multiply(x).subtract(q) ;
                        BigInteger rxqsqr = rxq.pow(2) ;

                        /*
                        * Newton itegration x-> x- [ (rx-q)^2-p^2D]/(2(rx-q)r).
                        * x-> x+ [p^2D- (rx-q)^2]/(2(rx-q)r).
                        */
                        BigInteger deltaxnum= p2D.subtract(rxqsqr) ;
                        BigInteger deltaxden= twor.multiply(rxq) ;
                        BigInteger deltax= deltaxnum.divide(deltaxden) ;

                        /* check whether the result is accurate. If the correction
                        * is zero, we may need to subtract 1 to get the
                        * floor of [p*sqrt(D)-q]/r. So the question is on
                        * the sign of [p^2D-(rx-q)^2)/[2r(rx-q)]
                        */
                        if ( deltax .compareTo(BigInteger.ZERO) == 0)
                        {
                                /* xnum/xden >= 1/2; 2*xnum >= xden
                                */
                                if ( deltaxnum.abs().shiftLeft(1).compareTo(deltaxden.abs()) >= 0)
                                {
                                        deltax = (deltaxnum.signum() == deltaxden.signum() )? BigInteger.ONE : BigInteger.ONE.negate() ;
                                }
                                else
                                {
                                        /* r>0 here, so rx-q needs to be a lower estimate
                                        * of p*sqrt(D), where p may have either sign:
                                        * rx-q <= p*sqrt(D)
                                        */
                                        if ( deltaxnum.signum() == 0 || deltaxnum.signum() == deltaxden.signum() )
                                                /* the correction tries to be positive, so we are on the lower side
                                                */
                                                return x;
                                        else
                                                return x.subtract(BigInteger.ONE);
                        ...
                        x= x.add(deltax) ;
                        /* excape infinitely oscillating solutions
                        */
                        if ( xstack[0] != null && xstack[0].compareTo(x) == 0)
                                return xstack[0].min(xstack[1]) ;
                        xstack[0] = xstack[1] ;
                        xstack[1] = x ;
                }
        } /* isqrt */

        /** Continued fraction of the square root of a positive integer
        * @param D The positive integer
        * @return The vector [a0,a1,a2,..] of sqrt(D) = a0+1/(a1+1/(a2+1/..)))
        * @sicne 2016-03-19
        */
        static Vector<BigInteger> cfracD(BigInteger D)
        {
                if ( D.compareTo(BigInteger.ZERO) < 0 )
                        throw new ArithmeticException("Negative argument "+ D.toString()) ;

                Vector<BigInteger> cfr = new Vector<BigInteger>();
                /* first a0 as the solution of isqrt(D)
                */
                BigInteger a0 = isqrt(D) ;
                cfr.add(a0) ;
                BigInteger a0sqr = a0.pow(2) ;
                /* nothing more to be done if D is a perfect square
                */
                if ( a0sqr.compareTo(D) < 0)
                {
                        /* sqrt(D)  = a0+1/(a1+....) means
                        * sqrt(D)-a0=1/(a1+..) means a1+.. = 1/[sqrt(D)-a0]
                        * = (sqrt(D)+a0)/[D-a_0^2]
                        */
                        BigInteger p = BigInteger.ONE ;
                        BigInteger q = a0 ;
                        BigInteger r = D.subtract(a0sqr) ;

                        /* remember q and r to detect periodicity of the recursion
                        * No need to remember p=1.
                        */
                        final BigInteger qstrt = q ;
                        final BigInteger rstrt = r ;

                        for(int i=0 ; ;i++)
                        {
                                BigInteger a = isqrt(D,p,q, r) ;
                
                                cfr.add(a) ;
                                /* a1+1/(a2+....)=[ sqrt(D)+a0]/(D-a_0^2) so
                                * 1/(a2+...) =[ sqrt(D)+a0]/(D-a_0^2) -a1
                                * =[ sqrt(D)+a0-a1(D-a0^2)]/(D-a_0^2), so
                                * a2+...=(D-a0^2)/[ sqrt(D)+a0-a1(D-a0^2)]
                                * =(D-a0^2)[sqrt(D)-a0+a1(D-a0^2)]/[ D-{a0-a1(D-a0^2)}^2]
                                * More generally a(j)+1/(a(j+1)+..)= (p*sqrt(D)+q)/r;
                                * 1/(a(j+1)+..) = (p*sqrt(D)+q-r*a(j))/r;
                                * a(j+1)+1/[a(j+2)+..] = r/(p*sqrt(D)+q-r*a(j))
                                * = r[p*sqrt(D)-q+r*aj]/(p^2*D-[q-r*aj]^2)
                                * pnew = r*p;
                                * qnew = r*(r*aj-q);
                                * rnew = p^2*D-(r*aj-q)^2;
                                */
                                BigInteger pnew = r.multiply(p) ;
                                final BigInteger tmp = r.multiply(a).subtract(q) ;
                                BigInteger qnew = r.multiply(tmp) ;
                                BigInteger rnew = p.pow(2).multiply(D).subtract(tmp.pow(2)) ;

                                final BigInteger gcd3 = pnew.gcd(qnew).gcd(rnew) ;
                                p = pnew.divide(gcd3) ;
                                q = qnew.divide(gcd3) ;
                                r = rnew.divide(gcd3) ;
        ...
        * @param D The factor in front of the y^2.
        * @return The fundamental solution represented as x/y.
        *  If there is none, returns null.
        * @since 2016-03-19
        */
        static Rational negPell (final BigInteger D)
        {
                /* calcuate the periodic palindromic continued fraction of sqrt(D)
                */
                Vector<BigInteger> cfr = cfracD(D) ;
                /* need to have odd length of the periodic part, which
                * means evenlength of the entire vector.
                */
                if ( cfr.size() % 2 == 0 )
                {
                        int i = cfr.size()-2 ;
                        Rational fsol = new Rational(cfr.elementAt(i)) ;
                        /* convergent down to a1
                        */
                        for( i = cfr.size()-3 ; i >= 0 ; i--)
                                fsol = fsol.invert().add(cfr.elementAt(i)) ;
                        return fsol ;
                }
                else
                        return null ;
