...
public class Permutation implements Cloneable
{
        ...
        */
        public Permutation(final int[] v) 
        {
                init(v) ;
        } /* ctor */
        ...
        * @since 2015-03-07
        */
        public Permutation multiply(final Permutation right)
        {
        ...
        */
        ...
        /** Cycle index
        * @param var the name of the base variable of the monomial
        * @return pairs of tthe form [cyclen1,nocy1],[cyclen2,nocy2]...
        * @since 2017-08-13
        */
        public Vector<int[]> cycIdx()
        {
                /* each pair of integers contains the cycle length and the number
                */
                Vector<int[]>cyc = new Vector<int[]>() ;
                int[][] allcyc = cycles() ;
                /* number of fixed elements (not listed in allcyc)
                */
                int fxel =images.length ;
                for(int c=0 ; c < allcyc.length; c++)
                {
                        final int clen = allcyc[c].length ;
                        /* is this cycle length already in cyc ?
                        */
                        boolean foundclen = false ;
                        for( int[] pair : cyc)
                        {
                                if ( pair[0] == clen)
                                {
                                        foundclen = true ;
                                        /* if yes, increase the exponent */
                                        pair[1] ++ ;
                                }
                        }
                        if ( !foundclen)
                        {
                                /* if not, add a new element with that base index */
                                int[] pair = new int[2] ;
                                pair[0] = clen ;
                                pair[1] = 1 ;
                                cyc.add(pair) ;
                        }
                        /* the number of elements not yet explicitly listed is decreased
                        * by the number of elements of that particular non-overlapping cycle
                        */
                        fxel -= clen ;
                }
                if (fxel > 0)
                {
                        /* if any fixed elements are in the cycle: add them too
                        */
                        int[] pair = new int[2] ;
                        pair[0] = 1 ;
                        pair[1] = fxel ;
        ...
        * The secondary key (if of the same length) is the left-right image=by-image comparison.
        * @param oth
        * @return true if the permutations are equal in length and in the order of elements.
        */
        public boolean equals(final Permutation oth)
        {
                if (images.length != oth.images.length)
                        return false;
                else
