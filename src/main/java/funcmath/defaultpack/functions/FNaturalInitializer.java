...
public class FNaturalInitializer extends Initializer {
  ...
  protected void initSimpleFunctions() throws NoSuchMethodException {
    SimpleFunction sum =
        new SimpleFunction("sum", FNatural.class.getMethod("sum", FNatural.class, FNatural.class));
    String sumHash = sum.getHash().toString();
    SimpleFunctionRegister.registerSimpleFunction(sum);

    SimpleFunction sub =
        new SimpleFunction("sub", FNatural.class.getMethod("sub", FNatural.class, FNatural.class));
    String subHash = sub.getHash().toString();
    SimpleFunctionRegister.registerSimpleFunction(sub);

    SimpleFunction mul =
        new SimpleFunction("mul", FNatural.class.getMethod("mul", FNatural.class, FNatural.class));
