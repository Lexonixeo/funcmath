...
public class FunctionMakerPanel extends GConsolePanel {
  ...
  private void tutorial() {
    out.clear();
    out.println("Обучение создания функции!");
    out.println("Для создания функции вам требуется совершить несколько действий:");
    out.println();
    out.println("1. Придумать название функции.");
    out.println("Для этого вам нужно ввести команду \"set name {название функции}\" (без скобочек)");
    out.println();
    out.println("2. Придумать описание функции (после знака =).");
    out.println("Для этого есть команда \"set description {описание}\"");
    out.println();
    out.println("3. Установить типы принимаемых аргументов.");
    out.println("В игре все числа (математические объекты) обладают типом (родом чисел).");
    out.println("Например, натуральные числа - natural, целые - integer.");
    out.println("С помощью модов можно создать свои типы чисел.");
    out.println("Для установки типов напишите команду \"set types {тип} {...}\" (вы можете ввести один или несколько типов через пробел)");
    out.println("(Пример использования: \"set types integer gaussian\" - установит для функции первым аргументом целое, а вторым - гауссовое целое число)");
    out.println();
    out.println("4. Установить определение функции.");
    out.println("Определением является выражение, состоящее из простейших функций, чисел-констант и аргументов.");
    out.println("Список простейших функций доступен по команде \"list\"");
    out.println("Числа-константы задаются с помощью выражения \"{тип}={число}\"");
    out.println("Аргументы пишутся как \"x{номер аргумента, начиная с 0}\"");
    out.println("Для установки опредения введите команду \"set definition {...}\"");
    out.println("(Пример использования: \"set definition f:sum:integer:integer x0 f:sum:integer:integer x1 x2\" - сумма трёх чисел чисел)");
    out.println("Если что, некоторые типы чисел могут конвертироваться из одних в другие");
    out.println();
    out.println("5. Установить количество использований функций.");
    out.println("Для этого введите команду \"set uses {число}\"");
    out.println();
    out.println("6. Сделать проверку функции.");
    out.println("Для системной проверки введите команду \"validate\".");
    out.println("Если вы хотите сами проверить работоспособность функции, введите \"use {название функции} {аргументы}\"");
    out.println("Аргументы задаются также, как и числа-константы.");
    out.println();
    out.println("7. Сохранить функцию.");
    out.println("Для сохранения функции вы можете написать команду \"save\"");
    out.println("Она будет сохранена в файле, который вы можете переименовать как угодно.");
    out.println("Файл будет сохранен в директории /data/functions/");
    out.println();
    out.println("Спасибо за прохождение обучения!");
    out.println();
  }
